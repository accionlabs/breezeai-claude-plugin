...
and does not read `apiKey` from `.breeze.json`. The stored `apiKey`
is used by non-MCP consumers (the ontology-generator CLI and the
REST upsert path).
...
`apiKey` from `.breeze.json`. The stored `apiKey` is used by non-MCP
consumers (the ontology-generator CLI and the REST upsert path).
