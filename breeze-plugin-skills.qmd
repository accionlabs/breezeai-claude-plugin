...
└── skills/
    ...
    ├── deprecated-cluster-pipeline/
    │   ├── SKILL.md                         ← (retired) v1 cluster pipeline — do not use
    │   └── generate.py                      ← Three-pass pipeline script (kept for resume of historical runs)
    ├── generate-code/
...
description: >
  ...
  Breeze project (UUID), checks ontology readiness, and optionally
  uploads repo or documents. No API key is collected here — skills
  that need one prompt on-demand. Use when: first time setup, "init
...
Authentication note: MCP tool calls authenticate via Keycloak OAuth —
Claude Code handles the sign-in automatically on the first MCP call
and does **not** read `apiKey` from `.breeze.json`. The few skills
that talk to non-MCP consumers (the ontology-generator CLI invoked
by `onboard-repository`, the REST upsert path used by the retired
`deprecated-cluster-pipeline`) prompt for the API key on-demand
with the generation URL — so this init step never asks for it.
...
Call `Call_Get_Project_Details_` with `uuid`.
...
## Step 3 — Report Status & Recommend Next Steps
...
<uiBaseUrl>/ontology/{projectUuid}/functional"
...
## Step 4 — Artifact Upload (if user chooses)

These commands take a `--user-api-key` flag. If `.breeze.json`
doesn't have `apiKey` yet, prompt for it first:

> Generate at **`<uiBaseUrl>/mcp/generate/key`** and paste here.

Save the pasted key to `.breeze.json` (no echo), then run the
upload. (In practice this responsibility now lives in
`/breeze:onboard-repository` and the retired
`/breeze:deprecated-cluster-pipeline` — this section is kept for
reference.)
...
`/breeze:init`. Extract `projectUuid`.
...
### breeze.config.json (plugin root, canonical defaults)
...
These are the URLs every skill renders to the user. The plugin ships
with these defaults; changing the values here repoints **every**
skill at a different Breeze deployment in one edit. Skills must read
these fields rather than hardcode URL literals.
...
```json
{
  ...
  "apiKey": "<BREEZE_API_KEY — added on-demand, not required at init>",
  "uiBaseUrl": "<optional override of breeze.config.json>",
  "apiBase": "<optional override of breeze.config.json>"
}
```

MCP tool calls authenticate via Keycloak OAuth — Claude Code performs
the sign-in flow on the first MCP tool call and does **not** read
`apiKey` from `.breeze.json`. The `apiKey` field is only populated
when a skill that needs it (`onboard-repository`, the retired
`deprecated-cluster-pipeline`) prompts the user. Pure MCP workflows
work with `projectUuid` alone.

**URL resolution order** (every skill follows this when rendering a
URL): `.breeze.json` field → `breeze.config.json` field → SKILL.md
fallback literal.
...
1. Read `.breeze.json`
2. If `projectUuid` is missing → tell user: "Run `/breeze:init` first."
3. Extract `projectUuid` for MCP tool calls.
4. If the skill also needs an `apiKey` (only `onboard-repository`
   and the retired `deprecated-cluster-pipeline`), check `.breeze.json` —
   if absent, prompt the user with the generation URL
   (`<uiBaseUrl>/mcp/generate/key`, resolved from `.breeze.json` →
   `breeze.config.json`), save it, and
   continue. Do not block at the guard for apiKey.
...
their personal `.breeze.json` (project UUID). The API key — needed
only by `onboard-repository` and the retired
`deprecated-cluster-pipeline` — is collected on-demand the first
time those skills run. This file is gitignored either way.
