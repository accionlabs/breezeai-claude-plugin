...
ENTRY_POINT:
  ...
  kind:                <route | panel | route-variant>
  title:               <human label>
...
2. For every imported component / hook / service / store / query, decide:
   - **Read it** if it contains any of: `useState`, `useReducer`, `useForm`, `useStore`, `onSubmit`, `onChange`, `validation`, `validate`, `schema`, `zod`, `yup`, `setError`, `mutate`, `fetch*`, `axios`, `apiFetch`, `dispatch(...Api`. **Angular equivalents:** `@Injectable`, `BehaviorSubject`, `Subject`, `Observable`, `FormGroup`, `FormBuilder`, `FormControl`, `HttpClient`, `MatDialog`, `MatDialogRef`.
   - **Read it** if it is a `Form*`, `*Form`, `Dialog*`, `Modal*`, `Drawer*`, `Sheet*`, `Panel*`, `Popover*`, `Popup*`, `Sidebar*`, `Navigation*`, `Stepper*`, `Wizard*`, `Tabs*`, `TabsContent*`, `*Layout` component **with its own state**. For Angular, the equivalent of "own state" is services with `@Injectable()`, components with `Signal`, `BehaviorSubject`, or injected services.
   ...
   - **Angular template pairing (mandatory when framework is Angular).** When reading any `.component.ts` file, immediately check for a `templateUrl` property in the `@Component({...})` decorator. If found, `Read` the referenced `.component.html` file — in Angular, ALL form fields, event bindings, conditional rendering (`*ngIf`, `@if`), iteration (`*ngFor`, `@for`), and two-way binding (`[(ngModel)]`) live in the HTML template, NOT in the TypeScript file. Skipping the template means skipping the entire UI surface. If the component uses an inline `template:` string instead of `templateUrl`, the template is already in the `.ts` file.
3. For every `setPanelType("X")` / disclosure-hook trigger you find, locate and `Read` the renderer. **Angular equivalents:** `this.dialog.open(`, `this.matDialog.open(`, `this.bottomSheet.open(`, `this.sidenav.open()`.
4. **Popup-trigger traversal (mandatory).** For every onClick handler, menu-item handler, or render-time conditional that opens a Popup / Modal / Dialog / Drawer / Sheet / Popover (signals: `setOpen`, `setIsOpen`, `setAnchorEl`, `openDialog`, `useDisclosure`, `<X open={…} onClose={…}>`, conditional render of `<*Dialog*>` / `<*Modal*>` / `<*Popup*>` / `<*Popover*>` / `<*Drawer*>`; **Angular signals:** `MatDialog.open(ComponentClass, config)`, `MatDialogRef`, `MAT_DIALOG_DATA`, `this.dialog.open(`, template `(click)="openDialog()"` / `(click)="openModal()"`), you MUST locate the popup's source component file and `Read` it before completing Phase 1. **Angular note:** for Angular, the popup component is typically identified by the first argument to `.open()` (it is a component class reference) — you should `Read` that component's `.ts` AND `.html` files. Do not assume "the popup just shows the same fields described at the trigger" — popups frequently contain entire sub-filter trees, include/exclude radio groups, multi-step sub-forms, or region/role-gated sections that are invisible from the trigger row alone. **If the popup component is referenced indirectly (imported by a child of SEED_FILE, not SEED_FILE itself), use `Grep` on the trigger's label / handler name / suspected component name (`*Popup`, `*Dialog`, `*Modal` under `src/components/`) to locate it.** Record every popup component file in `audit.filesRead` and enumerate its interactive elements as Actions per Phase 2 Pattern A. If a popup's contents cannot be located in code after a reasonable search, add an `audit.warnings[]` entry of type `popup_source_unresolved` naming the trigger and the searches attempted — do NOT fabricate or omit silently.
...
1. Identify the form boundary (`<form>`, `useForm`, `zodResolver`, or a submit handler). **Angular form boundaries:** `[formGroup]="formName"`, `(ngSubmit)="handler()"`, `FormBuilder`, `FormGroup`, `FormControl`, `Validators`, `[(ngModel)]="property"`. Note that Angular form boundaries are in the `.html` template file (not the `.ts` file), so reading the template is mandatory.
...
Apply to any JSX block that renders MORE THAN ONE displayed field as part of a repeated element OR a multi-field readout — e.g. a project card with name + description + tags + author + timestamp, a row in a results table, a detail panel with metadata, a header showing 4 stats. **For Angular apps, apply to any HTML template block (`.component.html`) that renders multiple fields via interpolation (`{{ value }}`), property binding (`[property]="value"`), or pipes (`value | pipe`).**
...
**How to detect a dispatcher:** look inside the form component body for `switch (label) {` / `if (type === 'X')` / `{label === 'persona' && <PersonaFields />}` / a `fieldsByType[type]` map / multiple `<Fields*>` subcomponents conditionally rendered. **Angular detection patterns:** `*ngSwitch` / `@switch` directives in templates, `[ngSwitch]="discriminator"` with `*ngSwitchCase="'value'"`, `@if (type === 'X')` blocks in templates, and TypeScript `switch` statements in component methods that conditionally set template state. Each distinct branch corresponds to one scenario in your output.
...
| `*ngIf="isAdmin"`, `@if (isAdmin)`, `[hidden]="!canEdit"`, `[disabled]="!hasPermission"`, `*appHasPermission="'ADMIN'"` | Angular template conditional rendering / custom structural directives |
| `canActivate: [AuthGuard]`, `canActivate: [RoleGuard]`, `@Inject(AUTH_TOKEN)` | Angular route guards and DI-based auth |

**Angular note:** visibility gates in Angular apps span TWO files — the component `.ts` file (where service calls like `this.authService.hasRole()` live) AND the `.html` template (where `*ngIf`/`@if`/`[hidden]`/`[disabled]` directives gate UI elements). You MUST scan BOTH files for gates.
...
1. Grep for hook usages: `useQuery`, `useMutation`, `useInfiniteQuery`, `fetchGet`, `fetchPost`, `fetchPut`, `fetchDelete`, `fetchPatch`, `apiFetch`, `axios.`, `api.`, `dispatch\(.*Api`. **Angular API patterns:** `HttpClient`, `this.http.get(`, `this.http.post(`, `this.http.put(`, `this.http.delete(`, `this.http.patch(`, `.subscribe(`, `this.apollo.query(`, `this.apollo.mutate(`, `` gql` ``.
2. For every hit, walk to the service file and `Read` it to extract the **literal URL**, **HTTP method**, **request shape**, **response shape**.
   - **Angular service resolution.** In Angular, API calls are typically encapsulated in `@Injectable()` service classes injected via the constructor (e.g., `constructor(private productService: ProductService)`). To find API URLs: (a) identify injected services from the component's constructor, (b) `Read` each service file, (c) locate `HttpClient` method calls (`.get()`, `.post()`, etc.) with literal URL strings. For GraphQL, look for Apollo service usage (`this.apollo.query()`, `this.apollo.mutate()`) or dedicated query/mutation files.
   - **Angular Apollo wrapper services.** Some Angular apps wrap Apollo in a custom service (e.g., `GraphQLService`) rather than calling `this.apollo.query()` directly. Domain services then call `this.graphQLService.query(...)` or `this.graphQLService.mutate(...)`. When you encounter an injected service that does NOT have `HttpClient` or `apollo` directly, check if IT injects another service that does — follow the chain (component → domain service → wrapper service → Apollo/HttpClient → URL/operation).
3. If the URL is a template literal, resolve it: substitute path params with `{paramName}`, query strings with `?key={value}`.
   - **GraphQL operations.** For GraphQL APIs, the HTTP endpoint is typically a single `/graphql` URL — recording only that is useless. Instead, record `apis[].url` as `Query.<operationName>` or `Mutation.<operationName>` (e.g., `Query.productDetails`, `Mutation.createEnquiry`). Extract the operation name from the `gql` tagged template literal (the word after `query` or `mutation` in the GraphQL document string). Set `apis[].method` to `QUERY` or `MUTATION` (not GET/POST). Set `apis[].type` to `"GraphQL"`. For `request`, list the GraphQL variables; for `response`, list the return type/shape from the query's type annotation or the `gql` response fields.
4. **Follow delegation all the way to the literal — not just one hop.** Trace every wrapper to the `fetch*`/`axios` literal: Redux thunk → service → URL; React-Query `useMutation`/`useQuery` → its `mutationFn`/`queryFn` → service → URL; Angular DI service → `this.serviceMethod()` → injected `HttpClient` → URL literal; Angular component `(ngSubmit)` handler → service method → `HttpClient.post()` → URL; **and any action that opens or delegates to a shared component (`*Form`, `*Dialog`, `*Modal`, `*Drawer`, or a toolbar/menu control) → `Read` that component → its `onSubmit`/`onSave`/`onConfirm` handler → the imported service function (resolve the import path) → the URL.** The submit endpoint frequently lives several hops away in a *different* file (e.g. a project-detail toolbar "Add to X" button → a shared `Form*` dialog → a service function in another module). Read across the hops; do not stop at the page.
...
6. **Rule B (mandatory):** every hook discovered in step 1 MUST resolve to a `Read` of a service file. For Angular, every injected service discovered via constructor DI MUST be `Read` to extract its API calls (HttpClient URLs or Apollo query/mutation operation names). If a hook or service reference is unresolved, go back and follow it before producing output.
...
5. ✅ Every imported component with state (`useState`/`useReducer`/`useForm`/`useStore`) was read? For Angular: every injected service with `@Injectable()`, every component with `BehaviorSubject`/`Signal`/`Observable` state, and every `.component.html` template paired with a read `.component.ts`?
...
14. ✅ **Hazard-family gate (core.md §6.1):** if any file read imports a `*-dialog`/`*-modal`/`*-drawer` family, a tab container, a lazy-mounted child, or a dispatcher — was each member (a) discovered via `Code_Graph_Search`, (b) inventoried via `Get_Code_Nodes_By_Label`, AND (c) `Read` for its field set, with one field-enumerated scenario per dialog? A hazard signal with zero graph calls, or an imported dialog neither read nor graph-resolved, is INVALID (unless `code_graph_unavailable`). For Angular: `MatDialog`/`MatBottomSheet` component families, `mat-tab-group` / `@switch` dispatchers, lazy-loaded child routes (`loadChildren`/`loadComponent`). For a true leaf, `audit.codeGraphSearches: []` is fine.
...
| 4 | **Forbidden UI words in action names** | Scan every action `name` for the forbidden UI words — the canonical list is `verbs.json → forbidden_ui_words` (in `SHARED_FUNCTIONAL_PATH`); `validate.py forbidden` is the authoritative gate. Angular-specific terms like `mat-dialog`, `mat-drawer`, `mat-sidenav`, `ngIf`, `ngFor` are also forbidden in action names (they are implementation details, not user-intent language). | Rewrite to platform-agnostic vocabulary. Common substitutions: `drop` → `upload`; `drop-zone` → `upload area`; `dialog` / `modal` → `overlay` or omit; `button` → `control` or omit; `dropdown` → `selector`; `panel` → `section`. Confirm semantics are preserved — never change what the action does, only its naming. |
