...
### Phase 3 — API / side-effect inventory (TWO boundary types)

A Web Forms action triggers a **postback** → a **code-behind event handler** → a **façade method** → … . That chain terminates in ONE of two boundaries — capture whichever applies. **Most monolithic Web Forms apps are Case B**, so do not assume SOAP.

**Case A — Network boundary (SOAP / WCF / ASMX web service).** The façade/manager delegates to a generated service-proxy that crosses a process boundary.
1. Grep the code-behind for façade/manager/service usages (`*Facade.`, `*Manager.`, `*Service.`, `*Client.`, injected `_service.`).
2. **Follow the chain to the service-proxy** and read it (`Reference.cs` / `Connected Services`; `SoapHttpClientProtocol` for ASMX, `ClientBase<T>`/`ChannelFactory<T>` for WCF). Extract the operation name, the endpoint (`.asmx`/`.svc` path or configured address / SOAPAction), and the request/response message DTOs.
3. Build the `Api`: `type: "SOAP"`, `method: "POST"` (or the `[WebInvoke]` verb), `url: <Service>.asmx/<Operation>` (ASMX) or `<IContract>/<Operation>` (WCF), `request`/`response` = the message DTOs.
4. **NEVER invent an operation.** The `url` MUST come from a proxy / service-reference / `.asmx`/`.svc` / WSDL you `Read` — never from the façade method name. If unreachable, leave `url` empty + an `api_url_unresolved` warning.

**Case B — In-process layered chain → data side effect (the COMMON case for a monolith).** The façade/service/repository are **in-process** (same solution, direct calls) and the action terminates in a **DB write** — ADO.NET `SqlCommand`/`ExecuteNonQuery`/**stored procedure**, EF `SaveChanges`, or a repository method — *not* a network call. Do NOT fabricate a SOAP node; capture the side effect instead:
1. **Follow the chain all the way down:** `control event → handler → *Facade.Method → *Service.Method → *Repository.Method → ADO.NET command / stored proc / EF entity`. Read each hop (on disk via imports; the code graph's `calls` edges confirm this chain is traceable). Do not stop at the façade.
2. Build the `Api` capturing the **data effect**: `type: "SQL"` (free text — also `"StoredProcedure"` / `"Database"`), `method:` the SQL verb or `"EXEC"`, `url:` the **stored-procedure name** or **table** written (e.g. `usp_CreateBUCCStudentEnrollment` or `StudentEnrollment`), `request`/`response`: params in / result out. If the exact proc/table can't be resolved (sparse statements), set `url` to the terminal `Repository.Method` and add an `audit.warnings[]` note.
...
- Every action whose first word is a NETWORK VERB MUST EITHER have a non-empty `apis[]` (a SOAP operation **or** a data-side-effect node — stored proc / table) **or** name the repository+table / stored proc in its `description`. Only if there is genuinely no side effect, rename to a non-network verb (`Capture …`, `View …`, `Open …`). Never leave a side-effect-verb action with no api and no identifier.
