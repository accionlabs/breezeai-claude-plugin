...
3. **Read-only fields** — distinguish incidental chrome from substantive record data:
   ...
   - **Substantive record fields on a detail/view surface** (where the displayed record fields ARE the primary content — e.g. an account/contact/case detail view showing name, type, industry, address, assigned user, …) → emit **one atomic `Review <field>` action per meaningful business field**, the read-mostly mirror of rule 2. Do **NOT** club a detail view's whole field set into a single `Review the … information` action with everything in the description — that destroys granularity exactly the way clubbing editable fields does. Use judgement to group only a tightly-coupled set (e.g. the three lines of one postal address = one `Review the billing address` action).
