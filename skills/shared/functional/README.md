...
| `human-overlay.md` | Rules that apply **only** to human personas: persona derivation, platform-agnostic action language + forbidden UI words, per-field atomicity, UI coverage. | ui pass, metadata human half, visual-to-text, analyze/update |
| `system-overlay.md` | Rules that apply **only** to `System` / `External System`: mechanical EP→persona map, required descriptions, one-operation-one-action (atomicity exempt), apis-OR-identifier `rule-a`. | backend pass, metadata system half |
...
- The three generator `validators/validate.py` are now **thin shims** that delegate to this `validate.py` (backend injects `--kind system`, ui injects `--kind human`, metadata auto-detects per half). All existing agent Phase-6 invocations work unchanged; `--kind` is optional (auto-detected from the single persona) and the engine exits 3 (degrade) when `jsonschema` is absent.
