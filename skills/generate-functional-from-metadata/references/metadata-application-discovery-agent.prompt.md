# Per-call inputs template — metadata discovery agent
...
Read the metadata mapping rules at RULES_PATH first.

Inventory every metadata-driven application (parse every `MAPL(*)`; also `MAAP`/`MASC` for batch) under
