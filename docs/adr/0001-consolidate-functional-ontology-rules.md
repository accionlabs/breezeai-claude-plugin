...
| Generator rich rules | `skills/generate-functional-from-{ui,backend,p3}/references/rules.md` + `schemas/upsert.schema.json` + `validators/validate.py` | apis[].type, citation regex, rule-a, field-coverage, atomicity, forbidden words | The **real** rulebook lives here, triplicated, and diverges between ui/backend/p3. |
...
5. **Three schemas, three validators** (`ui`/`backend`/`p3`) that are ~90% identical. The validators are **source-agnostic** (no MAPL/JSX strings; they read only `payload` + `audit`), so triplication is pure duplication.
...
- **Per-skill adapter** — source extraction only: P3 (`MAPL/MSCR/MFID`, widget codes `E/L/H/R/I/P/B`), UI (JSX/DOM/React-Query hooks), backend (routes/DTO/queues), aspx (SOAP/SQL). Each emits a **normalized `audit.declaredFields[]` + the two halves**, then runs the *same* validators.
...
skills/generate-functional-from-{ui,backend,p3}/
...
- Lift `core.md` + `human-overlay.md` + `system-overlay.md` from the P3 ruleset (most evolved).
- Shrink ui/backend/p3 `references/rules.md` to adapter tables only.
...
**Positive:** one place to change a rule; no Python/markdown divergence; the human/system split is explicit and matches reality; per-skill adapters stay small and source-focused; new source types (e.g. aspx, p3) plug in by writing an adapter + reusing the shared core/validator.

**Negative / risks:** package-time assembly adds a build step; C1–C5 require judgement calls; the Python validator import path must be kept working; a botched extraction could regress the P3 generation (mitigate: P3 is the model and is regression-checked against the live P3 project graph).
...
- Generator rich rules: `skills/generate-functional-from-{ui,backend,p3}/{references/rules.md, schemas/upsert.schema.json, validators/validate.py}`
- Flow agents: `agents/{spa,aspx,backend,p3}-flow-structuring-agent.md`, `agents/{backend-entrypoint,p3-application}-discovery-agent.md`
